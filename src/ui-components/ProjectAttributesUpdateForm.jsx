...
export default function ProjectAttributesUpdateForm(props) {
  const {
    id: idProp,
    projectAttributes: projectAttributesModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = projectAttributesRecord
      ? { ...initialValues, ...projectAttributesRecord }
      : initialValues;
    setRacNumber(cleanValues.racNumber);
    setFundingFiscalYear(cleanValues.fundingFiscalYear);
    setState(cleanValues.state);
    setCounty(cleanValues.county);
    setSubmittedBy(cleanValues.submittedBy);
    setCurrentDate(cleanValues.currentDate);
    setContactPhoneNum(cleanValues.contactPhoneNum);
    setContactEmail(cleanValues.contactEmail);
    setProjectLocation(cleanValues.projectLocation);
    setNationalForest(cleanValues.nationalForest);
    setForestServiceDistrict(cleanValues.forestServiceDistrict);
    setProjectLocationTownship(cleanValues.projectLocationTownship);
    setProjectGoal(cleanValues.projectGoal);
    setProjectDescription(cleanValues.projectDescription);
    setTypeLand(cleanValues.typeLand);
    setMeetPurpose(cleanValues.meetPurpose);
    setProjectType(cleanValues.projectType);
    setProjectAccomplishments(cleanValues.projectAccomplishments);
    setStartDate(cleanValues.startDate);
    setCompletionDate(cleanValues.completionDate);
    setListPartnernships(cleanValues.listPartnernships);
    setIdentifyCommunityBenefits(cleanValues.identifyCommunityBenefits);
    setIdentifyFederalBenefits(cleanValues.identifyFederalBenefits);
    setAccomplishmentMethods(cleanValues.accomplishmentMethods);
    setGenerateMerchantableTimber(cleanValues.generateMerchantableTimber);
    setAnticipatedCost(cleanValues.anticipatedCost);
    setOtherSourceOfFunding(cleanValues.otherSourceOfFunding);
    setMonitoringPlan(cleanValues.monitoringPlan);
    setRemediesForFailure(cleanValues.remediesForFailure);
    setErrors({});
  };
  const [projectAttributesRecord, setProjectAttributesRecord] = React.useState(
    projectAttributesModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ProjectAttributes, idProp)
        : projectAttributesModelProp;
      setProjectAttributesRecord(record);
    };
    queryData();
  }, [idProp, projectAttributesModelProp]);
  React.useEffect(resetStateValues, [projectAttributesRecord]);
  const validations = {
    racNumber: [],
    fundingFiscalYear: [],
    state: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ProjectAttributes.copyOf(projectAttributesRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ProjectAttributesUpdateForm")}
      {...rest}
    >
      <TextField
        label="Rac number"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || projectAttributesModelProp)}
